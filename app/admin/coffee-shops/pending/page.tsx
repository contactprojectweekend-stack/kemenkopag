...
export default async function AdminPendingPage() {
  ...
  const { data: pendingShops } = await supabase.from('coffee_shops').select('*').eq('status', 'pending');
      ...
      {(!pendingShops || pendingShops.length === 0) ? (
        ...
        <div className="space-y-3">
          {pendingShops.map((shop) => (
            <div key={shop.id} className="bg-white p-4 rounded-2xl border border-cream flex justify-between items-center">
              <div>
              ...
              <form action={handleApprove}>
                ...
                <button className="px-4 py-1.5 bg-emerald-700 text-white rounded-xl text-xs font-bold">
                  Approve (Verifikasi)
                </button>
              </form>
            </div>
